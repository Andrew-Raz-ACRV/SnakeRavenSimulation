function [Tend,Tendon_lengths] = PlotSnakeRavenTendons(isright,design,q)
%Plots a rendering of the snake like robot using Raven Kinematics
%in 3D plots as well as shows and computes the tendon lengths
%
% Inputs:
    ...
    for ii = 1:m
        ...
        if pan_first==true %Pan first case
            if isodd(ii) %pan disk for ii = 1,3,5,7...
                %Measure point A
                Pa = DetermineTendonPoints(Tend,w,alpha,d,0,1,col,k,segs);
                %Do transform
                Tend = Tend*Tpan;
                plotVariableNeutralLineDisk(Tend,w,alpha,d)
                %Measure Point B
                Pb = DetermineTendonPoints(Tend,w,alpha,d,1,1,col,k,segs);
                L = DrawTendons(Pa,Pb,col,L);
            else %tilt disk for ii = 2,4,6,8...
                Pa = DetermineTendonPoints(Tend,w,alpha,d,0,0,col,k,segs);
                Tend = Tend*Ttilt;
                plotVariableNeutralLineDisk(Tend*Ttiltrot,w,alpha,d)
                Pb = DetermineTendonPoints(Tend,w,alpha,d,1,0,col,k,segs);
                L = DrawTendons(Pa,Pb,col,L);
            end           
